...
/**
 *@todo: add cloudinary advanced image transformation and zustand state support
 */
export default function ImageCard({
  ...
  return (
    <TouchableOpacity onPress={handleRedirect} className="w-full h-[250px] my-2.5 mx-1.5 ">
        ...
        <View className="absolute flex-row  top-3 right-2">
          {iconBtns.map((item) => {
            if (item.type === "delete" && ownerId === userId) {
              return <IconBtn {...item} handlePress={handleDelete} key={item.type} />;
            }
            if (item.type === "download") {
              return <IconBtn {...item} handlePress={handleDownload} key={item.type} />;
            }
            if(item.type === "like") {
              return <IconBtn {...item} handlePress={handleLike} isLiked={isLiked} key={item.type} />;
            }
            return null
          })}
        </View>
